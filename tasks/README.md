...
# 11. GenTask Principle: Execution Manifest, Not Documentation Dump

A task file generated by GenTask must be a **lean execution manifest**:
- Tells the agent **what** to change and **where** to change it.
- Links to authoritative documentation in `docs/` and skills in `.ai/skills/` without copying their contents.
- Defines testable, binary acceptance criteria and required validation commands.
- Avoids redundant repetition across description, requirements, and notes sections.

---

# 12. Skill Budgeting & Limits

Every task must strictly limit its listed skills to avoid agent confusion and task bloat:

```text
Simple Task (isolated UI / tweak / fix):     2–4 skills
Normal Task (feature / component / event):   3–5 skills
Complex Task (multi-view / full playback):   5–7 skills
Hard Limit:                                  Maximum 7 skills
```

---

# 13. Task Decomposition Guidelines

When a planned feature or change requires **more than 7 skills** or crosses multiple architectural boundaries, GenTask must decompose the work into sequential, independently verifiable tasks:

```text
Example Decomposition for a New Battle Mechanic:
1. TASK-A: Backend domain & event emission (Domain / Application)
2. TASK-B: Realtime SignalR contract & serialization (Infrastructure / Api)
3. TASK-C: Client event projection & presentation state (GameRuntime / State)
4. TASK-D: Phaser visual presentation & animation playback (BattleScene / Views)
5. TASK-E: End-to-end integration & gameplay scenario testing
```
